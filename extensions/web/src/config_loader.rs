...
pub fn load_demo_config() -> Result<Option<Arc<DemoConfig>>, ConfigError> {
    let Some(demo_value) = load_config_section("demo.yaml")? else {
        return Ok(None);
    };

    let demo_config: DemoConfig =
        serde_yaml::from_value(demo_value).map_err(|e| ConfigError::Parse {
            config_name: "demo.yaml".to_string(),
            message: e.to_string(),
        })?;

    tracing::info!(
        page_count = demo_config.pages.len(),
        "Loaded demo config from config/demo.yaml"
    );

    Ok(Some(Arc::new(demo_config)))
}
