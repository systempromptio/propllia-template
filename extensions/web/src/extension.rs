...
use crate::demo::{DemoConfig, DemoPagePrerenderer};
...
static FEATURES_CONFIG: OnceLock<Result<Option<Arc<FeaturesConfig>>, ConfigError>> =
    OnceLock::new();
static DEMO_CONFIG: OnceLock<Result<Option<Arc<DemoConfig>>, ConfigError>> = OnceLock::new();
...
impl WebExtension {
    ...
    #[must_use]
    pub fn demo_config() -> Option<Arc<DemoConfig>> {
        DEMO_CONFIG
            .get_or_init(config_loader::load_demo_config)
            .clone()
            .inspect_err(|e| tracing::error!(error = %e, "Demo config error"))
            .ok()
            .flatten()
    }
}
...
impl Extension for WebExtension {
    ...
    fn page_prerenderers(&self) -> Vec<Arc<dyn PagePrerenderer>> {
        ...
        if let Some(config) = Self::demo_config() {
            for page in &config.pages {
                prerenderers.push(Arc::new(DemoPagePrerenderer::new(page.clone())));
            }
        }
